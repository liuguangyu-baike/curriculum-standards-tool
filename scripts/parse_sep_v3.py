...
def parse_sep_section(lines, start_idx, point_counters):
    ...
    while i < max_lines:
        ...
        # 遇到DCI的实际内容（以领域代码开头）结束
        if re.match(r'^[A-Z]{2,3}\d+\.[A-Z]:', line):
            print(f"    遇到DCI内容，结束SEP解析")
            break
            ...
            if line.startswith('▪'):
                ...
                while j < max_lines and lookahead < max_lookahead:
                    ...
                    if re.match(r'^[A-Z]{2,3}\d+\.[A-Z]:', next_line):
                        break
                    # 如果遇到标题，停止
                    if next_line in ["Disciplinary Core Ideas", "Crosscutting Concepts"]:
                        break
